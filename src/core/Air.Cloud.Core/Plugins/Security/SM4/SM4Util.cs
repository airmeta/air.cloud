...
namespace Air.Cloud.Core.Plugins.Security.SM4
{
    public class SM4Util
    {
        ...
        internal SM4Util sm4_setkey_enc(SM4Parameter ctx, byte[] key)
        {
        ...
        internal SM4Util sm4_setkey_dec(SM4Parameter ctx, byte[] key)
        {
            int i = 0;
        ...
        internal byte[] sm4_crypt_ecb(SM4Parameter ctx, byte[] input)
        {
        ...
        internal byte[] sm4_crypt_cbc(SM4Parameter ctx, byte[] iv, byte[] input)
        {
            var Entrypt = new Func<byte[]>(() =>
            {
                List<byte> bousList = new List<byte>();
                //如果是加密 则在加密前添加偏移量
                if (ctx.isPadding) input = padding(input, SM4_ENCRYPT);
                int i = 0;
                int length = input.Length;
                byte[] bins = new byte[length];
                Array.Copy(input, 0, bins, 0, length);
            ...
            if (ctx.mode == SM4_ENCRYPT) return Entrypt(); else return Decrypt();
        }
    }
