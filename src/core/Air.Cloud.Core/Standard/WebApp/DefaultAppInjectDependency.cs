...
namespace Air.Cloud.Core.Standard.WebApp
{
    [IgnoreScanning]
    public class DefaultAppInjectDependency : IAppInjectStandard
    {
        public IConfiguration LoadConfiguration(string ConfigurationName, bool IsCommonConfiguration)
        {
            ...
            action();
            if (config == null) throw new System.Exception("加载外部配置文件" + ConfigurationName + "失败");
            if (token != null)
            {
                //监视文件变更
