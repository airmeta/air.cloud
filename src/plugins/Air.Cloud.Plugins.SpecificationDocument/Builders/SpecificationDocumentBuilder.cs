...
namespace Air.Cloud.Plugins.SpecificationDocument.Builders;
...
public static class SpecificationDocumentBuilder
{
    ...
    {
        // 载入配置
        _specificationDocumentSettings = AppConfiguration.GetConfig<SpecificationDocumentSettingsOptions>("SpecificationDocumentSettings", true);
    ...
    private static void CreateGroupEndpoint(SwaggerUIOptions swaggerUIOptions)
    {
        foreach (var group in DocumentGroups)
        {
            ...
            swaggerUIOptions.SwaggerEndpoint($"/{routeTemplate}", groupOpenApiInfo?.Title ?? group);
        }
    }
    /// <summary>
    /// 获取分组信息缓存集合
    ...
    {
        // 获取所有的控制器和动作方法
        var controllers = AppCore.EffectiveTypes.Where(u => Penetrates.IsApiController(u));
        if (!controllers.Any())
        {
