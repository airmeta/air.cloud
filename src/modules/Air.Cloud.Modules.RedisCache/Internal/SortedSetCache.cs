...
namespace SSS.Modules.Redis.Service
{
    ...
    public class SortedSetCache :ISortedSetCache
    {
        ...
        /// <returns></returns>
        public bool Add<T>(string key, T value, double? score = null) => Redis.SortedSetAdd(key, AppRealization.JSON.Serialize(value), score ?? _GetScore(key));
        ...
        public long Add<T>(string key, List<T> value, double? score = null)
        {
            ...
            SortedSetEntry[] rValue = value.Select(o => new SortedSetEntry(AppRealization.JSON.Serialize(o), scoreNum++)).ToArray();
        ...
        {
            var sValue = AppRealization.JSON.Serialize(startValue);
            var eValue = AppRealization.JSON.Serialize(endValue);
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public double? Score<T>(string key, T value) => Redis.SortedSetScore(key, AppRealization.JSON.Serialize(value));
        ...
        public Dictionary<T, double> RangeByRankWithScores<T>(string key, long start = 0, long stop = -1, bool desc = false) where T : class, new()
        {
            ...
            {
                dicList.Add(AppRealization.JSON.Deserialize<T>(item.Element), item.Score);
            }
            return dicList;
        }
        ...
        /// <returns></returns>
        public double Decrement<T>(string key, T value, double scores) => Redis.SortedSetDecrement(key, AppRealization.JSON.Serialize(value), scores);
        /// <summary>
        ...
        /// <param name="scores"></param>
        /// <returns></returns>
        public double Increment<T>(string key, T value, double scores) => Redis.SortedSetIncrement(key, AppRealization.JSON.Serialize(value), scores);
        #endregion
        ...
        /// <param name="score">排序分数，为空将获取集合中最大score加1</param>
        /// <returns></returns>
        public async Task<bool> AddAsync<T>(string key, T value, double? score = null) => await Redis.SortedSetAddAsync(key, AppRealization.JSON.Serialize(value), score ?? _GetScore(key));
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public async Task<double?> SetScoreAsync<T>(string key, T value) => await Redis.SortedSetScoreAsync(key, AppRealization.JSON.Serialize(value));
        ...
        /// <param name="scores"></param>
        /// <returns></returns>
        public async Task<double> DecrementAsync<T>(string key, T value, double scores) => await Redis.SortedSetDecrementAsync(key, AppRealization.JSON.Serialize(value), scores);
        /// <summary>
        /// 修改指定Key和值的Scores在原值上增加scores，并返回最终Scores
        /// </summary>
        /// <typeparam name="T"></typeparam>
        ...
        /// <param name="scores"></param>
        /// <returns></returns>
        public async Task<double> IncrementAsync<T>(string key, T value, double scores) => await Redis.SortedSetIncrementAsync(key, AppRealization.JSON.Serialize(value), scores);
        #endregion
